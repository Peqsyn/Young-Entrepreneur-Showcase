...
# if both are external cameras its probably VideoCapture(2)
...
def run_webcam():
    ...
    while(True):
        ...
        if image2 is not None:
        	cv2.imshow('video 2 output', image2)
        ...
        if keyPress == 32: #this is the ASCII key for the space bar
            ...
            take_picture(image_counter2, image2)
            call_googlevision()
...
def take_picture(image_counter, image):
    ...
    for i in range(1):
    	print(timestr)
    	print(file+timestr)
    	if not os.path.exists(file + timestr):
    		os.makedirs(file + timestr)
        image_name = "image{}.jpeg".format(image_counter)
        if (os.path.exists(file + timestr + image_name)):
            os.remove(file + +timestr + image_name)
        cv2.imwrite(file + timestr + image_name, image)
